...
import { getRawAvatarPath } from '../util/avatarUtils';
...
import { hasExpiration, VERSION_NEEDED_FOR_DISPLAY } from '../types/Message2';
...
/*
  ...
  [June 2024] This has been extracted from `./conversations.ts` and modified so that it 
  does not refer to types from `window.*` like this:
    ...
    Doing so is a problem because we are seeking to define these at runtime -- `window.Signal` is undefined
    at compile time.
...
export class ConversationModel extends Model<ConversationAttributesType> {
  ...
  contactCollection?: Collection<ConversationModel>;
  ...
  getContactCollection(): Collection<ConversationModel> {
    const collection = new Collection<ConversationModel>();
  ...
  async loadNewestMessages(
    ...
    try {
      ...
      if (newestMessageId) {
        const newestInMemoryMessage = await window.Signal.Data.getMessageById(
          newestMessageId
        );
      ...
      const metrics = await window.Signal.Data.getMessageMetricsForConversation(
        {
          conversationId,
  ...
  async loadOlderMessages(oldestMessageId: string): Promise<void> {
    ...
    try {
      const message = await window.Signal.Data.getMessageById(oldestMessageId);
      if (!message) {
      ...
      const models = await window.Signal.Data.getOlderMessagesByConversation({
        conversationId,
        includeStoryReplies: !isGroup(this.attributes),
        limit: MESSAGE_LOAD_CHUNK_SIZE,
  ...
  async loadNewerMessages(newestMessageId: string): Promise<void> {
    ...
    try {
      const message = await window.Signal.Data.getMessageById(newestMessageId);
      if (!message) {
        throw new Error(
      ...
      const receivedAt = message.received_at;
      const sentAt = message.sent_at;
      const models = await window.Signal.Data.getNewerMessagesByConversation({
        conversationId,
        includeStoryReplies: !isGroup(this.attributes),
        limit: MESSAGE_LOAD_CHUNK_SIZE,
        receivedAt,
  ...
  async loadAndScroll(
    ...
    try {
      ...
      const { older, newer, metrics } =
        await window.Signal.Data.getConversationRangeCenteredOnMessage({
          conversationId,
          includeStoryReplies: !isGroup(this.attributes),
          limit: MESSAGE_LOAD_CHUNK_SIZE,
          messageId,
  ...
  async cleanModels(
    ...
    for (let max = result.length, i = 0; i < max; i += 1) {
      ...
      if ((schemaVersion || 0) < VERSION_NEEDED_FOR_DISPLAY) {
  ...
  async addKeyChange(
    ...
    return this.queueJob(`addKeyChange(${keyChangedId})`, async () => {
      ...
      const message: MessageAttributesType = {
        ...
        readStatus: ReadStatus.Read,
        seenStatus: SeenStatus.Unseen,
        schemaVersion: VERSION_NEEDED_FOR_DISPLAY,
      };
  ...
  async sendStickerMessage(packId: string, stickerId: number): Promise<void> {
    ...
    const data = await window.Signal.Migrations.readStickerData(path);
  ...
  async enqueueMessageForSend(
    ...
    const attributes = await window.Signal.Migrations.upgradeMessageSchema({
      id: generateGuid(),
      timestamp: now,
      type: 'outgoing',
      body,
    ...
    if (sticker) {
      await window.Signal.Data.addStickerPackReference(
        model.id,
        sticker.packId
      );
    }
    ...
    if (decrypted) {
      const newAttributes = await Conversation.maybeUpdateProfileAvatar(
        this.attributes,
        {
          ...
          doesAttachmentExist: window.Signal.Migrations.doesAttachmentExist,
        }
      );
  ...
  async destroyMessagesInner({
    ...
    if (source === 'local-delete' && isEnabled('desktop.deleteSync.send')) {
      ...
      const addressableMessages =
        await window.Signal.Data.getMostRecentAddressableMessages(this.id);
  ...
  async notify(
    ...
    const isExpiringMessage = hasExpiration(messageJSON);
  ...
  async getAvatarOrIdenticon(): Promise<{
    ...
    if (avatarPath) {
      return {
        url: window.Signal.Migrations.getAbsoluteAttachmentPath(avatarPath),
        absolutePath:
          window.Signal.Migrations.getAbsoluteAttachmentPath(avatarPath),
      };
    }
    ...
    return {
      url,
      absolutePath: path
        ? window.Signal.Migrations.getAbsoluteTempPath(path)
        : undefined,
    };
  }
